  ...
  async post(req, res, next) {
    ...
    try {
      ...
      if (status == "1") {
        ...
        await new Promise(function (resolve, reject) {
          db.query(
            `UPDATE Zayavka SET step=4,paid_status='waiting',term='${term}',limit_summa='${summa}',scoring_end = CURRENT_TIMESTAMP WHERE id = ${id};`,
            function (err, results, fields) {
              if (err) {
                resolve(null);
                return null;
      ...
      try {
        ...
        let zayavka = await new Promise(function (resolve, reject) {
          db.query(
            `SELECT Zayavka.*,fillial.name,fillial.percent_type from Zayavka,fillial WHERE Zayavka.id=${id} and fillial.id=Zayavka.fillial_id `,
            function (err, results, fields) {
              if (err) {
                resolve(null);
                return null;
